...
```
```{r}
write_rds(acc,"data/rds/acc.rds")#to save the file to rds
```

```{r}
acc<-read_rds("data/rds/acc.rds") #re-reading the acc file to the environments
```
...
```{r}
write_rds(SAB,"data/rds/SAB.rds")#to save the file to rds
```
